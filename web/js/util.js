...
function util_processDateAndTime(dateSelector, timeSelector) {
  var date = $.trim($(dateSelector).val());
  var time = $.trim($(timeSelector).val());
  if (date.length > 0 && time.length > 0) {
    var formattedDate = dateFormat(date, "MMM dd, yyyy"); 
    var formattedTime = dateFormat(date, "kk:mm:ss z"); 
  }
