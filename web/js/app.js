...
var app_currentDMECodeId;
var app_currentTxCodeId;
var app_currentImagingTestId;

var app_currentDentalProcId;
var app_currentLabTestId;
var app_currentDrugItemId;
...
function getStaticLists() {
  $.post("app/getStaticLists", {}, function(data) {
    ...
    app_countries = parsedData.countries;
    app_triageCodes = parsedData.triageCodes;
    app_mdbs = parsedData.mdbs;
    app_dispositions = parsedData.dispositions;
 });
}
...
function initPatientSearchTypeAheads() {
  ...
  $.post("app/getPatientSearchTypeAheads", {data:jsonData}, function(data) {
    ...
    $('#patient-search-patient-name').typeahead(
      { hint: true, highlight: true, minLength: 1 },
      { name: 'patientNames', displayKey: 'value', source: util_substringMatcher(patientSearchTypeAheads.patientNames) }); 
...
function patientSearch() {
  ...
  var jsonData = JSON.stringify({ 
    ...
    patientNameFilter: $.trim($("#patient-search-patient-name").val()),
...
function viewClinicianMessage() {
  ...
  function getPatientChart() {
    ...
    $.post("app/getPatientChart", {data:jsonData}, function(data) {
      ...
      app_patientChartCountry = parsedData.country;
      ...
      $('.patient-chart-country').html(app_patientChartCountry);
