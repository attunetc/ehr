...
function setEncounterFormModes(hasOwnership) {
  ...
  setEncounterFormMode(app_currentEncounter, 'ed', app_currentEncounter.edSaved, hasOwnership);
}
...
function initDxTypeAheads(id) {
  var dxCode = new Bloodhound({
    ...
    remote: {
      url: 'app/searchICD10AM?sessionId='+clinician.sessionId+'&searchText=%QUERY',
      filter: function (data) {
        return $.map(data.icd10AMList, function (dxCode) {
          return { value: dxCode.code + ' ' + dxCode.description };
        });
      }
    }
  });
  dxCode.initialize();
  $('.icd10AM-typeahead').typeahead( { hint: true, highlight: true, limit: 10, minLength: 3 },
...
function initEncounterTypeAheads() {
  var jsonData = JSON.stringify({ clinicianId: clinician.id, sessionId: clinician.sessionId });
  $.post("app/getEncounterTypeAheads", {data:jsonData}, function(data) {
    var parsedData = $.parseJSON(data);
    encounterTypeAheads = parsedData.encounterTypeAheads;
    setDMETypeAheads();
    setImagingCodeTypeAheads();
  });
}



function setDMETypeAheads() {
  $('.dme-code-typeahead').typeahead(
    { hint: true, highlight: true, minLength: 3 },
    { name: 'dmeCodes', displayKey: 'description', source: util_substringMatcher(encounterTypeAheads.dmeCodes) }); 
}


function setImagingCodeTypeAheads() {
  $('.imaging-code-typeahead').typeahead(
    { hint: true, highlight: true, minLength: 3 },
    { name: 'imagingCodes', displayKey: 'description', source: util_substringMatcher(encounterTypeAheads.imagingCodes) }); 
}
...
function saveExamEncounterForm(encounter) {
  ...
  for (i=0;i<encounter.dxCodes.length;i++) { 
    ...
    encounter.dxCodes[i].icd10AM = $("#encounter-icd10AM-"+dxCodeId).val();
  }
  ...
  encounter.exam.visitType.id = $.trim($("#encounter-visit-type-"+id).val());
  encounter.exam.serviceCategory.id = $.trim($("#encounter-service-category-"+id).val());
...
function saveEDEncounterForm(encounter) {
  var id = encounter.id;  
  encounter.ed.presentationDate = util_processDateAndTime('#encounter-ed-pres-date-'+id, '#encounter-ed-pres-time-'+id);
  encounter.ed.commencementDate = util_processDateAndTime('#encounter-ed-serv-date-'+id, '#encounter-ed-serv-time-'+id);
  encounter.ed.episodeEndDate = util_processDateAndTime('#encounter-ed-episode-end-date-'+id, '#encounter-ed-episode-end-time-'+id);
  encounter.ed.seeAndTreat = $('input[name=encounter-ed-sat-completed-'+id+']:checked').val() == 'true';
  encounter.ed.satDxCode = $.trim($("#encounter-ed-primary-sat-icd10AM-"+id).val());
  encounter.ed.primaryDxCode = $.trim($("#encounter-ed-primary-icd10AM-"+id).val());
  encounter.ed.disposition = $("#encounter-ed-disposition-"+id).val();
  encounter.ed.triageCode = $("#encounter-ed-triage-code-"+id).val();
  encounter.ed.mbd = $("#encounter-ed-mbd-"+id).val();
  
  encounter.edSaved = true;
  jsonData = JSON.stringify({ 
    sessionId: clinician.sessionId, 
    encounter: encounter
  });
  $.post("patient/createED", {data:jsonData}, function(data) {
    renderEncounterFormSection (encounter, 'ed', true, true);
  });
}
...
function updateDxCode(icd10AM, dxCodeId) {
  var jsonData = JSON.stringify({ 
    sessionId: clinician.sessionId, 
    dxCodeId: dxCodeId,
    icd10AM:icd10AM
...
function updateDMECode(dmeCode, dmeCodeId) {
  var jsonData = JSON.stringify({ 
    sessionId: clinician.sessionId, 
    dmeCodeId: dmeCodeId,
    dmeCode:dmeCode
  });
  $.post("patient/updateDMECode", {data:jsonData}, function(data) {
  }); 
}



function updateImagingTest(imagingTest, imagingTestId) {
  var jsonData = JSON.stringify({ 
    sessionId: clinician.sessionId, 
    imagingTestId: imagingTestId,
    imagingTest:imagingTest
  });
  $.post("patient/updateImagingTest", {data:jsonData}, function(data) {
  }); 
}


function updateDrugItems(drugItem, drugItemId) {
  var jsonData = JSON.stringify({ 
    sessionId: clinician.sessionId, 
    drugItemId: drugItemId,
    drugItem:drugItem
  });
  $.post("patient/updateDrugItem", {data:jsonData}, function(data) {
  }); 
}


function updateDentalProc(dentalProc, dentalProcId) {
  var jsonData = JSON.stringify({ 
    sessionId: clinician.sessionId, 
    dentalProcId: dentalProcId,
    dentalProc:dentalProc
  });
  $.post("patient/updateDentalProcedure", {data:jsonData}, function(data) {
  }); 
}


function updateLabTest(labTest, labTestId) {
  var jsonData = JSON.stringify({ 
    sessionId: clinician.sessionId, 
    labTestId: labTestId,
    labTest:labTest
  });
  $.post("patient/updateLabTest", {data:jsonData}, function(data) {
  }); 
}
...
function getCurrentDMECodeId(e) {
  var attributes = e.currentTarget.attributes;
  for (i=0;i<attributes.length;i++) {
    if (attributes[i].name == 'data-id') {
      app_currentDMECodeId = parseInt(attributes[i].nodeValue); 
    }
  }
} 


function getCurrentImagingTestId(e) {
  var attributes = e.currentTarget.attributes;
  for (i=0;i<attributes.length;i++) {
    if (attributes[i].name == 'data-id') {
      app_currentImagingTestId = parseInt(attributes[i].nodeValue); 
    }
  }
} 


function getCurrentLabTestId(e) {
  var attributes = e.currentTarget.attributes;
  for (i=0;i<attributes.length;i++) {
    if (attributes[i].name == 'data-id') {
      app_currentLabTestId = parseInt(attributes[i].nodeValue); 
    }
  }
}

function getCurrentDentalProcId(e) {
  var attributes = e.currentTarget.attributes;
  for (i=0;i<attributes.length;i++) {
    if (attributes[i].name == 'data-id') {
      app_currentDentalProcId = parseInt(attributes[i].nodeValue); 
    }
  }
}

function getCurrentDrugItemId(e) {
  var attributes = e.currentTarget.attributes;
  for (i=0;i<attributes.length;i++) {
    if (attributes[i].name == 'data-id') {
      app_currentDrugItemId = parseInt(attributes[i].nodeValue); 
    }
  }
}
...
function deleteDMECode(element) {
  var id = element.attr('name');
  var jsonData = JSON.stringify({ sessionId: clinician.sessionId, dxCodeId:id});
  $.post("patient/deleteDMECode", {data:jsonData}, function(data) { 
    $('#dme-code-'+id).remove();
  });  
}
...
function deleteImagingTest(element) {
  var id = element.attr('name');
  var jsonData = JSON.stringify({ sessionId: clinician.sessionId, imagingTestId:id});
  $.post("patient/deleteImagingTest", {data:jsonData}, function(data) { 
    $('#imaging-test-'+id).remove();
  });  
}


function deleteLabTest(element) {
  var id = element.attr('name');
  var jsonData = JSON.stringify({ sessionId: clinician.sessionId, labTestId:id});
  $.post("patient/deleteLabTest", {data:jsonData}, function(data) { 
    $('#lab-test-'+id).remove();
  });  
}

function deleteDrugItem(element) {
  var id = element.attr('name');
  var jsonData = JSON.stringify({ sessionId: clinician.sessionId, imagingTestId:id});
  $.post("patient/deleteDrugItem", {data:jsonData}, function(data) { 
    $('#drug-item-'+id).remove();
  });  
}

function deleteDentalProc(element) {
  var id = element.attr('name');
  var jsonData = JSON.stringify({ sessionId: clinician.sessionId, imagingTestId:id});
  $.post("patient/deleteDentalProcedure", {data:jsonData}, function(data) { 
    $('#dental-proc-'+id).remove();
  });  
}
